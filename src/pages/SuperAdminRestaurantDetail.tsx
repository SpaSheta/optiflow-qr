...
const planBadge = (plan: string | null) => {
  ...
  return <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${c[p] ?? c.starter}`}>{p.charAt(0).toUpperCase() + p.slice(1)}</span>;
};
...
const statusBadge = (status: string | null) => {
  ...
  return <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${c[s] ?? c.active}`}>{s.charAt(0).toUpperCase() + s.slice(1)}</span>;
};
